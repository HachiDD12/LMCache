...
class BlendServer:
    ...
    def get_stitched_tokens_for_msg_content(self, msg_content: str, blend_tokens: List[int]) -> List[int]:
        ...
        If min_chunk_size > 0, blend_tokens are only added after accumulating
        at least min_chunk_size tokens. This allows merging small chunks together.
        
        Also tracks chunk sizes for distribution analysis if chunk_dist_file is set.
        """
        tokens = []
        # split on blend special string
        msg_chunks = [c for c in msg_content.split(self.blend_special_str) if c.strip()] # remove empty chunks
        ...
        if self.min_chunk_size <= 0:
            for chunk in msg_chunks:
                ...
                if self.chunk_dist_file:
                    self.chunk_sizes.append(len(chunk_tokens))
                actual_chunks += 1
        else:
            # enforce min_chunk_size before adding blend tokens
            accumulated_tokens = []
            for chunk in msg_chunks:
                chunk_tokens = self.tekkenizer.encode(chunk, bos=False, eos=False)
                accumulated_tokens.extend(chunk_tokens)
                
                if len(accumulated_tokens) >= self.min_chunk_size:
                    # flush accumulated tokens and add blend tokens
                    tokens.extend(accumulated_tokens)
                    tokens.extend(blend_tokens)
                    # Track accumulated chunk size
                    if self.chunk_dist_file:
                        self.chunk_sizes.append(len(accumulated_tokens))
                    accumulated_tokens = []
                    actual_chunks += 1
            
            if accumulated_tokens:
                # if new acc not big enough merge with previous chunk
                if len(accumulated_tokens) < self.min_chunk_size:
                    tokens = tokens[:-len(blend_tokens)]
                    actual_chunks -= 1
                tokens.extend(accumulated_tokens)
                tokens.extend(blend_tokens)
                # Track final accumulated chunk size
                if self.chunk_dist_file:
                    self.chunk_sizes.append(len(accumulated_tokens))
                actual_chunks += 1
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Minimum chunk size (in tokens) before adding blend tokens. Chunks smaller than this will be accumulated. (default: 0, meaning every chunk gets blend tokens)"
    )
    parser.add_argument(
        "--chunk-dist-file",
        type=str,
